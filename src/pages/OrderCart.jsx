...
export default function OrderCart() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // 1. Submit Order to Express + Mongoose Backend (MongoDB)
      const orderPayload = {
        items: cart.map(item => ({
          productId: item.id,
          name: item.name,
          ...
          specs: {
            material: item.specs?.material || "PLA Premium",
            dimensions: item.specs?.dimensions || "Standard Scale"
          }
        })),
        totalAmount: cartTotal,
        deliveryAddress: {
          phone: userData?.phone || "",
          addressLine: userData?.address || notes || "Checkout comments specified on WhatsApp"
        }
      };

      console.log("OrderCart: Logging order details to localStorage...");
      const orderNumber = `PMX-DEV-${Date.now().toString().slice(-4)}`;
      const newOrder = {
        id: `order_${Date.now()}`,
        orderNumber,
        ...
        status: "pending",
        user: currentUser ? {
          name: userData?.fullName || currentUser.displayName || "Guest Customer",
      ...
      // 3. Redirect to WhatsApp (Appended with MongoDB Order Number)
      const whatsappNumber = "919832769269";
      let message = `Hello PROMETHIX3D,\nI have placed a new order (No: ${orderNumber}).\n\n`;
      message += `Name: ${userData?.fullName || currentUser?.displayName || "Customer"}\n`;
      if (userData?.phone) message += `Phone: ${userData.phone}\n`;
      if (userData?.address) message += `Delivery Address: ${userData.address}\n`;
