...
export default function AdminDashboard() {
  ...
  const deleteOrder = (orderId) => {
    ...
    try {
      const storedOrders = localStorage.getItem("promethix_orders");
      let orders = storedOrders ? JSON.parse(storedOrders) : [];
      orders = orders.filter(order => (order.id || order._id) !== orderId);
      localStorage.setItem("promethix_orders", JSON.stringify(orders));
  ...
  const handleAddOfflineOrder = (e) => {
    ...
    try {
      const orderNumber = `PMX-OFF-${Date.now().toString().slice(-4)}`;
      const newOrder = {
        id: `order_${Date.now()}`,
        orderNumber,
        items: cartItems,
        ...
        status: "delivered", // Offline orders default to delivered
        user: {
          name: customerName,
          email: `${customerName.toLowerCase().replace(/\s+/g, '')}@offline.com`
        }
      };
