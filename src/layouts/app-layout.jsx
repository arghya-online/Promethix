...
function AppLayout() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-body bg-[#ffffff] text-slate-900 relative overflow-hidden">
      <ScrollToTop />

      {/* Announcement Bar - Clean High-Contrast Dark Bar */}
      {showAnnouncement && (
        <div className="bg-zinc-950 text-white text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] py-2.5 px-4 text-center relative z-50 shadow-sm border-b border-zinc-900">
          <span>Free Shipping on All Orders Above ₹999</span>
          <button
            onClick={() => setShowAnnouncement(false)}
            className="absolute right-4 top-1/2 -translate-y-1/2 p-1.5 text-zinc-400 hover:text-white transition-colors rounded-full border-0 cursor-pointer"
          >
